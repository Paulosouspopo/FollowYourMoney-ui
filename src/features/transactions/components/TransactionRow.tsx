...
export function TransactionRow({ tx, onClick, showAsset = false }: { tx: TransactionResponse; onClick?: () => void; showAsset?: boolean }) {
  ...
  return (
    <button type="button" onClick={onClick} disabled={!onClick}
      aria-label={onClick ? `Modifier : ${TRANSACTION_TYPE_LABEL[tx.type]} ${isManualSymbol(tx.symbol) ? tx.assetName : tx.symbol} du ${formatDate(tx.transactionDate)}` : undefined}
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium truncate">
          {TRANSACTION_TYPE_LABEL[tx.type]}{showAsset && ` · ${isManualSymbol(tx.symbol) ? tx.assetName : tx.symbol}`}
        </p>
